...
class ProductsController extends Controller
{
    ...
    public function addToCart(Request $request) {
        ...
        if($addCart) {
            return Redirect::route("single.product", $request->pro_id)->with(['success' => 'The item has been added']);
        }
    }

    public function cart() {
        $itemsInCart = Cart::select()->where('user_id', Auth::user()->id)->get();
        $subTotal = Cart::where('user_id', Auth::user()->id)->sum('subtotal');
        return view('products.cart',compact('itemsInCart', 'subTotal'));
    }
    ...
    public function deleteFromCart($id) {
        ...
        $deleteItem->delete();
        if($deleteItem) {
            return Redirect::route("products.cart")->with(['delete' => 'The item has been deleted']);
        }
    }
    ...
    public function checkout() {
        $checkoutProducts = Cart::select()->where('user_id', Auth::user()->id)->get();
        $checkoutSubtotal = Cart::select()->where('user_id', Auth::user()->id)->sum('subtotal');
        return view('products.checkout', compact('checkoutProducts', 'checkoutSubtotal'));
    }
